    ...
    let usersTrails = this.state[name].map((trail, i) => {
      ...
      return (
        <div key={i}>
          ...
          <Link
            key={i}
            to={`/Trail/${trail.saved_trail_id || trail.visited_trail_id}`}
          >
            <div className="usersTrailsTrailContainer">
              <div className="usersTrailImageContainer">
                <img
                  className="usersTrailImage"
    ...
    return (
      <div className="usersTrailsContainer">
        {location.pathname === "/Your Saved Trails" ? (
          <div>
            <h1
              style={{ fontSize: "36px", margin: "25px 0" }}
            >{`Trails you have saved for later`}</h1>
          </div>
        ) : (
          <div>
            <h1
              style={{ fontSize: "36px", margin: "25px 0" }}
            >{`Trails you have visited`}</h1>
          </div>
        )}
