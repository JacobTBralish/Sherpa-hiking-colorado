    ...
    let mappedWeather = findWeatherInfoByDay(weather, getDate, getTime).map(
      (day, index) => {
        let mappedDay = day.hour.map((info, index) => {
          return (
            <div className="weatherInfoBox" key={index}>
              <h4 id="weatherTime">{info.time}</h4>
              <ul className="dropdownContent">
                <li>
                  <img
                    alt={info.description}
                    src={`http://openweathermap.org/img/w/${info.icon}.png`}
                  />
                </li>
        ...
        return (
          <div className="weatherMain" key={index}>
            <h3>{day.day}</h3>
    ...
    return (
      <div className="weatherTableContainer">
        {error ? (
          <div>
            {" "}
            Oh no!There was an error loading the trails.Please try again later.{" "}
          </div>
