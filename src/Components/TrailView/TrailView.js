  ...
  componentDidMount() {
    axios
      .get(
        ...
        }&key=200356963-c67e8738e2f605aeb5bcc2a5ef5f6375`
      )
      .then(res => {
        ...
        <div key={i} className="trailViewContainer">
          ...
          <div className="trailViewInfo">
              ...
              {!user ? (
                <div className="trailsViewButtonCluster">
                  <button
                    onClick={() =>
                      alert(
                        "You must be logged in to add this to your visited list!"
                      )
                    }
                  >
                    Visited
                  ...
                  <button
                    onClick={() =>
                      alert("You must be logged in to save this for later!")
                    }
                  >
                    Save for later
                  </button>
            ...
            <div className="trailViewSummaryContainer">
              ...
              <h3 className="trailViewSummary">
                {trail.summary.length
                  ? trail.summary
                  : "It looks like nobody has left a summary on this trail yet!"}
              </h3>
              {/* { trail.summary.length > 0 ? '' : <div id='addSummaryButton'><button onClick={() => {}}>Add A Summery</button></div> } */}
              <div>
                <h3 className="trailViewSummary">
                  {trail.high && trail.low
                    ? `The hike has an evelvation that starts at ${
                        trail.low
                      }ft and ends at ${
                        trail.high
                      }ft, which is an elevation change of ${trail.high -
                        trail.low}ft.`
                    : null}
                </h3>
              </div>
              <div className="trailSummarySubContainer">
                <div>
                  <label id="trailLabelFix" htmlFor="trailLength">
                    Length:
                  </label>
